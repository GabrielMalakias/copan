defmodule Copan.Consumers.Base do
  defmacro __using__(_options) do
    quote do
      ...
      * `:name` - used for name registration
      ...
      def rabbitmq_connect(%{config: config, module: module, reconnect_attempt: attempt} = state) do
      ...
      def bind_queue(chan, queue, exchange, routing_key) when is_bitstring(routing_key) do
        Queue.bind(chan, queue, exchange, routing_key: routing_key)
      end

      def bind_queue(chan, queue, exchange, routing_keys) when is_list(routing_keys) do
        bind_queue = fn(routing_key) -> Queue.bind(chan, queue, exchange, routing_key: routing_key) end

        Enum.each(routing_keys, bind_queue)
      end
    end
  end
end
